...
| D49 | **The introspector records three indexes on columns the tables no longer have.** `idx_supply_chain_data_plant` is recorded `ON supply_chain_data (plant)`, and `20250822025432` RENAMEd `plant` to `plant_name`; Postgres renames an index's column reference with the column, so production's index is on `plant_name` and the artifact's is on a column that does not exist. `supply_chain_data_multi_tier` has two more (`material_id`, `higher_level_component_id`). The artifact is therefore wrong about three indexes, and the error is invisible to every gate that compares the artifact against the MIGRATIONS — because the migrations do say what the artifact says. Found by D31's rehearsal, which cannot apply them and skips each with a named warning. Low consequence today (an index is a performance fact, not an access one) and it is the introspector's ALTER-handling that is incomplete, which is the part worth fixing. Found by WP 3.0's gap check. **CLOSED (WP 6.2) — AND THE THREE ARE NOT ONE DEFECT, WHICH THIS ROW'S STATED CAUSE GETS WRONG FOR TWO OF THEM.** `idx_supply_chain_data_plant` is the rename case exactly as described, and `renameColumnEverywhere` now follows a `RENAME COLUMN` into indexes, this table's own constraint text, index predicates, and the TARGET column list of any foreign key elsewhere that points at the renamed column — the column-rename analogue of the table-rename fix D52 already had. **The two on `supply_chain_data_multi_tier` were never renamed and never existed**: that table was created 2025-09-08 without `material_id` or `higher_level_component_id`, the indexes were written 2025-09-09, and no migration has ever added the columns — so those statements raised `42703` when they ran (`IF NOT EXISTS` guards the INDEX existing, not the column) and the indexes are not in the database. That is **D48's** class, not D52's. The introspector now refuses to record an index naming a column its table lacks, lists it under `invalid_indexes` with the migration that wrote it, and prints the count — not silent, for the same reason `unparsed` is not. Only a BARE identifier is checked: `created_at DESC` carries a sort direction and `md5(triple::text)` is an expression. The three rehearsal skip-warnings are gone in all three modes. **`introspectorDependentObjects.test.ts` gates both halves separately** and each mutation is caught: stop following the rename (3 red), record the impossible indexes again (3 red). The `invalid_indexes` list may not GROW — a third would be a migration about to abort on deploy | `scripts/data-contract/introspect.mjs` (`renameColumnEverywhere`, the index-validity pass); `src/lib/policies/__tests__/introspectorDependentObjects.test.ts` | **CLOSED (WP 6.2)** |
...
| D59 | **A CHECK written INLINE on a column is invisible to the artifact, so the rehearsed database does not have it and the generated page does not publish it.** `introspect.mjs` reads a column's type, its NOT NULL and its DEFAULT and drops the rest; only a NAMED, table-level `ADD CONSTRAINT … CHECK` is recorded. The artifact holds **24 CHECK constraints across 15 tables** while the migrations contain **253 `CHECK (` occurrences** — most of that gap is repetition across shadowed definitions, but `ingest_files` alone loses three real ones (`source_kind`'s vocabulary, `byte_size >= 0`, and the SHA-256 shape). Two consequences, and the second is worse: `contract:rehearse` builds a database with no such constraint, so an assertion that a bad value is REFUSED passes when it is run against the migration and fails when it is run against the artifact; and `docs/data/tables/*.md` renders a table's CHECK list, so a rule that rejects a user's upload appears in no document (§5 T1). **Found by the third rehearsal mode on WP 3.2's own branch** — green fresh and green over production's shape, red against its own artifact, which is precisely the case the WP 3.1 follow-up added that mode for. Same family as D49 (a column rename not followed into indexes) and D52 (a table rename not followed into foreign keys): the introspector is incomplete about DEPENDENT objects, one kind at a time. WP 3.2 walks around it — `20260916000014` writes every CHECK as a named table-level constraint — rather than relying on it being fixed. **CLOSED (WP 6.2), and the artifact goes from 24 CHECK constraints to 84.** The count in this row needed the same sanity check every scan in this package has: 255 `CHECK (` occurrences is NOT 231 missing constraints — most is repetition across shadowed definitions. The real loss is **64 inline column-level CHECKs across 20 migrations**, which `parseColumn` dropped because it read a type, a NOT NULL and a DEFAULT and nothing else. Parsed with `parenBody` rather than a regex, because `CHECK (status IN ('a','b'))` defeats `/CHECK\s*\(([^)]*)\)/` on the first nested paren. **LIFTED, not carried as a column flag** — `liftImplicitConstraints` already does this for PRIMARY KEY and UNIQUE so a later `DROP CONSTRAINT` can find them, and CHECK needs it for the same reason: `20260721000001` drops `ai_chat_events_event_kind_check` and re-adds a WIDER vocabulary, so a flag-carried original would have survived its own removal and the rehearsed database would enforce a rule production dropped. **MAKING IT REAL IMMEDIATELY CAUGHT FOUR REHEARSALS WRITING ROWS PRODUCTION WOULD REJECT** — `030` (`user_files.kind = 'report'`, not in the vocabulary), `110` and `170` (`triggered_by = 'schedule'`/an email, where the CHECK allows `manual`/`scheduled`), and `140`, whose column list and VALUES list had been MISALIGNED since the file was written: `'{}'::jsonb` was going into `proposals.provenance`, a text column with a three-value vocabulary. Those assertions were being made against rows that could not exist. That is this row's own stated harm arriving from the other direction | `scripts/data-contract/introspect.mjs` (`parseColumn`'s `parenBody` read, `liftImplicitConstraints`'s CHECK arm, the `ADD COLUMN` lift); `src/lib/policies/__tests__/introspectorDependentObjects.test.ts` | **CLOSED (WP 6.2)** |
...
### WP 6.2 — Fix the divergences *(D17, D18, D34, D47, D48, D49, D53, D58, D59, D66, D69, D71; D16 closed in WP 0.1)*
...
### WP 6.2 (slice 9) — The dependent-object class, fixed as a class · 2026-09-18 · no migration

**What slice 8 promised.** Its gap check said D49 is the fourth instance of one
root cause — the introspector is incomplete about DEPENDENT objects — and that
"the pattern is now four instances and deserves one fix rather than four." This
slice takes that seriously, and then finds the framing was itself half wrong.

**D49 names three indexes. They are two different defects.**

`idx_supply_chain_data_plant` is the rename case the row describes: `20250822025432`
renamed `plant` to `plant_name`, Postgres carried the index across, and the
artifact kept indexing a column that no longer exists. `renameColumnEverywhere`
now follows a `RENAME COLUMN` into every dependent object it can touch —
indexes, index predicates, this table's own constraint text, and the TARGET
column list of a foreign key in any other table. That is the column-rename
analogue of the table-rename fix D52 already had, written once for the class
rather than once for the measured instance.

**The two on `supply_chain_data_multi_tier` have nothing to do with renames.**
That table was created 2025-09-08 **without** `material_id` or
`higher_level_component_id`; the indexes were written 2025-09-09; no migration
has ever added the columns. `CREATE INDEX IF NOT EXISTS … ON t(material_id)` on a
table with no such column raises `42703` — `IF NOT EXISTS` guards the index
existing, not the column — so those indexes are **not in the database** and never
were. That is **§4 D48's** class: a statement that aborted in production and
nothing ever said so.

Had the fix followed D49's stated cause, two thirds of it would have gone to the
wrong place. The row is corrected in §4 rather than left to mislead the next
reader.

**The introspector now refuses to record an impossible index**, lists it under
`invalid_indexes` with the migration that wrote it, and prints the count — not
silent, for the same reason `unparsed` is not. Only a BARE identifier is checked:
`created_at DESC` carries a sort direction and `md5(triple::text)` is an
expression, and a naive check flags 30 false positives on this artifact. All
three rehearsal skip-warnings are gone, in all three modes.

**Gated, with slice 8's lesson applied first rather than promised.**
`introspectorDependentObjects.test.ts` covers both halves independently, and both
mutations are caught before the fix was called done: stop following the rename
(3 assertions red), record the impossible indexes again (3 red). The
`invalid_indexes` list may not GROW — these two are historical and unfixable
without rewriting a migration nobody should rewrite, but a **third** would be a
statement somebody is about to ship that aborts on deploy.

**Gap check.** Three findings:

1. **The dependent-object class is now three-quarters closed and the last quarter
   is D59.** D52 (table rename → foreign keys) ✅, D53 (schema qualifier) ✅,
   D49 (column rename → indexes, plus impossible indexes) ✅. D59 — an inline
   `CHECK` is never recorded at all — is still *walked around* rather than fixed:
   WP 3.2 writes every CHECK as a named table-level constraint, which works for
   new migrations and leaves the 253 existing `CHECK (` occurrences unread.
2. **`20250909153130` probably aborted ENTIRELY, and this slice only proves two
   statements of it did.** In PostgreSQL a failed statement aborts its
   transaction, so the third index in that file — `idx_…_project_level`, whose
   columns do exist — is equally absent from production. The artifact still
   records it and nothing here can tell. §15's schema probe is the instrument
   that would settle it, and it is the same shape as D48.
3. **Nothing checks a CONSTRAINT for the same impossibility.** The index pass
   validates bare column names; a `UNIQUE (col)` or a named `CHECK` naming a
   column the table lacks would be recorded exactly as these indexes were. Not
   observed today, and not gated either.

**Still open in WP 6.2:** D18, D34, D48, D51, D58, D59, D66, D71, D87, D94, D95,
D96.

### WP 6.2 (slice 10) — The last quarter of the dependent-object class · 2026-09-18 · no migration

**What slice 9 promised.** That the introspector's incompleteness about DEPENDENT
objects is one bug with four instances — D52, D53, D49 closed, **D59 the last
quarter, "still walked around rather than fixed."** This closes it.

**The count needed the same sanity check every scan in this package has.** This
row reads "24 CHECK constraints in the artifact while the migrations contain 255
`CHECK (` occurrences," and its own text warns most of that gap is repetition.
Measured properly: **64 inline column-level CHECKs across 20 migrations** are the
real loss. `parseColumn` read a type, a NOT NULL and a DEFAULT and dropped the
rest, so only a NAMED table-level `ADD CONSTRAINT … CHECK` was ever recorded.

Parsed with `parenBody`, not a regex: a CHECK body contains nested parentheses,
quoted strings and casts, and `CHECK (status IN ('a','b'))` defeats
`/CHECK\s*\(([^)]*)\)/` on the first one.

**Lifted, not carried.** `liftImplicitConstraints` already materialises a
column's PRIMARY KEY and UNIQUE as named constraints so a later `DROP CONSTRAINT`
can find them — its own docstring explains why. CHECK needed the same treatment
for the same reason: `20260721000001` drops `ai_chat_events_event_kind_check` and
re-adds a **wider** vocabulary. Carried as a column flag, the original would have
survived its own removal and the rehearsed database would enforce a rule
production dropped.

**── WHAT MAKING IT REAL IMMEDIATELY CAUGHT ────────────────────────────────**

Four rehearsals started failing, because their fixtures had been writing rows
production would reject and nothing had ever checked:

| file | the row it wrote |
|---|---|
| `030` | `user_files.kind = 'report'` — the vocabulary is `report_xlsx`, `report_pdf`, `export_csv`, `upload` |
| `110` | `triggered_by = 'schedule'` — the vocabulary is `manual`, `scheduled` |
| `170` | `triggered_by = '<an email>'` — written by **this package**, in slice 8 |
| `140` | **`'{}'::jsonb` into `proposals.provenance`**, a text column with a three-value vocabulary |

`140` is the one worth reading. Its column list and its VALUES list had been
misaligned since the file was written — `payload` and `provenance` both received
`'{}'::jsonb`, and everything after shifted by one. The rows those assertions
were made against **could not exist in production.** That is this defect's own
stated harm arriving from the other direction: an assertion made against a
database missing the constraint proves nothing about one that has it.

**Two mutations, and the second is the one that matters**: stop parsing the
inline CHECK (2 red), and parse it but do not LIFT it (3 red — including the
column/constraint symmetry, which is the assertion that would have caught a
flag-carried original surviving its DROP).

**My own gate found a gap in my own fix.** The symmetry assertion failed on
`projects.data_type`: `ALTER TABLE … ADD COLUMN … CHECK (…)` parses the check
onto the column, and lifting ran only for `CREATE TABLE` bodies. The `ADD COLUMN`
path now lifts too — 83 → 84 constraints. Two further assertions of mine were
also wrong and were corrected rather than the code: the surviving
`ai_chat_events` definition is identified by `implicit` rather than by a guessed
substring of a vocabulary that keeps changing, and a lifted constraint is matched
by COLUMNS rather than by a computed name, because Postgres does not rename
constraints when a table is renamed — `ingest_runs` still carries
`erp_sync_runs_triggered_by_check`, faithfully.

**The dependent-object class is now closed**: D52 (table rename → foreign keys),
D53 (the schema qualifier), D49 (column rename → indexes, plus impossible
indexes), D59 (inline CHECKs). Four defect rows, one root cause, four instances.

**Gap check.** Three findings:

1. **WP 3.2's walk-around is now redundant and should be retired deliberately.**
   It writes every CHECK as a named table-level constraint because inline ones
   were invisible. They are not any more. The convention is harmless and the
   REASON for it is gone, so it should either be restated on its own merits
   (named constraints are droppable by name) or dropped — not left as a rule
   whose justification has quietly expired.
2. **Nothing validates a CHECK's expression against the table's columns.** Slice
   9 added that for indexes. A `CHECK (col_that_does_not_exist > 0)` would be
   recorded and would abort on deploy exactly as the two invalid indexes did.
   Same shape, not yet gated.
3. **Four fixtures wrote impossible rows for as long as they have existed**, and
   the only reason it surfaced is that a constraint became real. Nothing checks
   a rehearsal's fixture against the vocabulary of the column it writes — the
   database does it now, which is the right place, but only for constraints the
   artifact carries.
