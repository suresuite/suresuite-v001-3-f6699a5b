...
| **D105** | **Twelve of eighteen edge functions are not deployed by CI, and four of them carry WP 4.3's dual-write.** `supabase-functions.yml` deploys SIX functions by name — `project-ai-chat`, `project-ai-health`, `agent-apply`, `report-render`, `sim-command`, `api` — and its `paths:` filter lists only those directories plus `_shared/**`. The other twelve, among them `calculate-network-science-metrics`, `calculate-node-prominence`, `predict-critical-nodes`, `combine-project` and **`ingest-file`**, have no deploy step and no path trigger. `git log -S` over the workflow finds **no commit that ever added one**: it is not a step that was removed, it is a step that has never existed. **What it costs is three packages' worth of false conclusions.** WP 3.2's §15 section reads "No CSV has been uploaded through `ingest-file` yet"; WP 3.4's reads "0 of 6 972 canonical rows trace to a source line"; WP 4.2's reads "Zero runs, as expected"; and D88 concluded "the precondition is adoption, not code". Every one of those is the same fact — the function is not deployed — read as a statement about users. **An undeployed edge function is the worst shape a gap can take**: it does not 404 and it does not error, it answers as its previous version, so the feature appears to work, no test goes red, no log complains, and the only visible symptom is a table that stays empty. §16 · WP 4.3's claim that the analyzers "did not reach production until run #50" is wrong in the same direction: run #47 (2026-09-17T22:17Z) succeeded and deployed the six named functions; the analyzers were in neither. **Found by executing it** — a user ran the network analysis and §15 run `35399391429` read the store minutes later: 0 runs, 0 results, 9 016 of 9 016 derived rows with no input hash, byte-identical to the run eleven minutes before | `.github/workflows/supabase-functions.yml` — its `paths:` list and its `functions deploy` steps — against `ls supabase/functions`; §15 runs `35398461411` and `35399391429` | **WP 6.3** *(the four analysis functions are deployed by this row's own commit, which is what D88 needs. The other eight are NOT: `ingest-file` alone would activate the whole server-side landing path — parse, tier-0 storage, staged rows, the diff screen, the promotion — in production on merge, and that is a change to make deliberately with a §15 reading on each side of it, not a line added in passing to a workflow)* |
