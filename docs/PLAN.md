...
shared preamble (`assert_writer_may_act`) that sets `app.current_user_id` LOCAL and
refuses below project role `editor`:
...
**Sixteen mutations, every one red with its own message.** Six against the
...
connection already held), six against the RPCs (remove the role check, remove the
NULL-actor check, disable the target whitelist, promote `removed_upstream` rows,
accept a cross-project score set, and the composite again), and two against the
ratchet (a 27th unattributed writer; `snapshot_dataset` losing its GUC).
...
#### J · Baseline numbers
  ...
  - **Sixteen mutations**, every one red with its own message, and one of them
