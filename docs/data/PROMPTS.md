...
on branch claude/busy-lovelace-5hxsh6 (PR #187). Read §0 of that plan first —
...
2. STAY IN SCOPE. The plan's §2 lists 20 defects, each assigned to a WP. If you
   find one belonging to another WP, record it in §12 and move on. Do not fix it.
...
4. Spend the last ~10% of your budget on the GAP CHECK. Append your findings
   and your Handoff note to §12 Drift Log in the SAME commit as the code.
...
· The provenance logic at StagePolicyTable.tsx:1170-1225 is a VERBATIM copy of
  ...
  them is WP 6.2's job — do not do it here, but note in §12 that they must stay
...
End by running EVERY query in §11 of the plan against one real project and
recording the counts in §12. Those numbers are the baseline every later phase is
...
```
Implement WP 0.3 from docs/data/IMPLEMENTATION-PLAN.md.

Already verified (re-check before relying on it):
· docs/ has 27 files; scsim/docs/ has 14 and is GENERATED by
  scsim/scripts/gen_docs.py — link to it, never move or edit it by hand.
· src/pages/help/docBodies.tsx is 2781 lines of hand-written JSX carrying a large
  amount of reference content that duplicates code. Do NOT touch it in this WP —
  it is Phase 5. But DO inventory which of its facts are duplicates; that
  inventory is this WP's most important output.
· /help and /help/:slug currently route to NotFound (App.tsx:212-214).

This is a move-and-label WP, not a rewrite. Preserve content verbatim; the only
new prose is the status banners and docs/data/README.md.

Leave a tombstone stub at every old path — other docs and possibly the app link
to them.
```
...
· Price has NO variability field anywhere in scsim — SupplierLink.cost and
  ...
  to the engine RFC in §10. Do NOT attempt to add one; that is an engine change.
...
Add the invariants table from §1 of the plan to CLAUDE.md in this commit.
```
...
over org deny, expired grant) into §12 as part of the gap check.
```
...
table and tick whether it audits. Un-audited paths become a WP 3.x addendum in §12.
```
...
· Duplicates therefore already exist in real projects. Run the §11 duplicate query
...
· Duplicates distort sourcing_ratio (combine-project:277 — the duplicate appears in
  ...
  numbers. That is the D5 damage being undone — record the before/after in §12 so
  nobody later mistakes it for a regression.
· Normalization at promotion (invariant I3) is the point of this WP. After it, no
  consumer converts. WP 5.3 will remove the downstream conversions; note in §12
...
incomplete — record that in §12 rather than papering over it here.
```
...
· Per-page reads confirmed by the audit are listed in §6 of the figures artifact and
...
```
Implement WP 5.2 from docs/data/IMPLEMENTATION-PLAN.md.

Already verified (re-check before relying on it):
· src/components/docs/DocsLayout.tsx (408 lines) already exists and is responsive.
  Reuse it; do not write a second shell.
· src/pages/help/HelpPage.tsx routes through docBodies.tsx (2781 lines). Per the
  approved decision, KEEP its narrative and generate its reference half.
· /help and /help/:slug currently route to NotFound (App.tsx:212-214). Restoring
  them is part of this WP.
· docBodies.tsx already assumes five audience groups (Overview / For Users / For
  Modelers / For IT / Reference). Map those onto the capability catalog from
  20260711000002_unified_access_control.sql rather than inventing a second taxonomy.

Desktop and mobile must read ONE payload. If you find yourself writing a mobile
content tree, stop — that is the duplication this whole phase exists to end.

Work the Phase-0 duplicate list in §12 to zero, and say so explicitly in the
handoff note.
```
...
· Browser-side computation to remove: ProcessLevelNetwork reachability and level
  ...
  — those are WP 6.2.
· The downstream unit conversions made dead by WP 3.3 should be listed in §12.
  Remove them here.
...
1. Read §12 Drift Log for the last entry and any partial handoff note.
...
· Re-read the previous WP's Handoff note in §12. Did reality match?
· Re-run this WP's Exit checks. Report honestly — a check that passes only with a
  caveat has not passed.
· Run the §11 verification SQL if this WP touched data, and compare to the recorded
  baseline.
...
5. Update §13 Sequencing if effort estimates moved.
