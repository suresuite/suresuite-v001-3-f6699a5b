...
| **G19** | **Inventory is not trackable over time, and the one chart that claims to show it shows half of it.** *(added with the WP 9.1 inventory workstream)* The engine measures material stock (`on_hand_value`) and finished-goods stock (`fg_value`) on adjacent lines of the same weekly block, and BOTH are in the golden trace — but `ScenarioResult.extra_series`, which is what becomes `run_replications.time_series`, named three series in a dict literal and `fg_value` was not one of them. So the /policies panel captioned "Inventory dynamics" plotted material stock alone and called it inventory, finished goods reached no user at all, and neither stock was available in UNITS. Below that, the only finer grain was the per-item inspection panel (G17), which needs `full_debug` and exactly one replication and is therefore absent from every run a user actually makes — and on /simulation-lab it was empty by construction, because that page dispatches no inspection run. The root cause is `single-source` one layer below markdown: "which weekly series exist" was authored six times, and the gate written after G17/§4 D113 to catch exactly this read one of the six | `scsim/scsim/core/engine.py` (`extra_series`), `scsim/scsim/io/traces.py` (`trace_frame`), `scripts/data-contract/chains.mjs::deriveReplicationSeries`, `src/components/policies/RunValidateStage.tsx` | A planner cannot see the stock their own inventory policies produce — P-P.1's coverage weeks and P-P.3's safety stock are tuned against a chart that omits every finished good — and cannot see it in units at all. **Closed by WP 9.1** (PLAN.md §19): one declared vocabulary (`WEEKLY_SERIES`), the two missing unit series, and a shared inventory panel on both result surfaces. **NOT** closed by node- or echelon-level rollup, which this engine cannot measure — inventory is one bucket per material and one per product, so any such figure would be an allocation presented as a measurement (§4 D113's lesson) |
...
| **Effective-economics display + Data map grid** — the editor and /policies surface render the engine's §4 fallback chains live: NULL master economics show the derived value (cheapest inbound `unit_price` for cost, demand-weighted outbound `unit_price` for price) with a provenance badge instead of appearing "missing"; a read-only **Data map** tab lists every uploaded column of the six worker-read datasets with its engine destination and live status (used / fallback-active / default-applied / unused) | G4 usability — no duplicate finance entry when logistics carry prices | Frontend encoding of the mapping contract lives in `src/lib/policies/effectiveEconomics.ts` + `src/lib/policies/dataMap.ts`, kept in lockstep with `project_map.py` (see `docs/data-simulation-mapping.md` §8). Masters stay the override layer; nothing is copied into them |
