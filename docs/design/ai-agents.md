...
1. **Internal reducers** *(exist today)*: the `grading.ts::REDUCERS` library (`cheapest_inbound_price`, `demand_weighted_outbound_price`, …) — data-derived from the project's own rows; manifest grade `info`.
...
3. **`rate_io_technical_coefficient@1`** *(family `benchmark_scaled`, role `prior`)* — sector input–output technical coefficients as priors: `r̂ = a × u_p ÷ c_m`, where `a` is the monetary technical coefficient (EUR of input sector per EUR of output production) from the **checked-in seed slice** `_shared/ioCoefficientsSeed.json` (`slice_version`ed like `leiSeed.json`; rows name Eurostat FIGARO Supply-Use/IO, OECD ICIO and EXIOBASE datasets + vintages verbatim; seed values pending owner-confirmed refresh, flagged in the file header; never fetched at runtime), and the price ratio `u_p / c_m` converts money shares to physical units (`u_p` = master `sell_price` else demand-weighted outbound; `c_m` = master `cost` else cheapest inbound — the engine's own fallback chains). **Sector assignment is a deterministic token match** of the material name against each row's `match_tokens` (file order; the manufacturing-composite row is the declared fallback) — never fuzzy, never scored. Interval = the row's [low, high] × the same price ratio; basis `prior_range`. LCA process compositions (ecoinvent, EU EF), ECHA SCIP physical-unit content, customs quantity fields and pilot IMDS/BOM data remain named future `prior`/`verify` sources for this method family — NOT wired (the §18.5 entitlement discipline).
