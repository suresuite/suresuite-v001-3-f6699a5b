...
| **4** *(later)* | *Extend* — new intelligence | B7 Cost Estimator, B8 Network Cartographer, B9 Disruption Sentinel (§18), background-execution addendum (§18.4) | Q26–Q28 RESOLVED at v1.5 (methodology papers + source registry, §18.5) — designs are buildable in order B7 → B8 → B9; still gated on staging capacity, licensed-source entitlement verification (§18.5), and Q2 resolved principals for anything scheduled |
