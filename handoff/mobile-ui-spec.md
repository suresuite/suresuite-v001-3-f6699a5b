...
| Radius | all 4px — see note | all 4px | all 4px |
...
> **Radius note (verified 2026-09-06).** `tailwind.config.ts` now maps
> `borderRadius.lg`, `.md` and `.sm` **all** to `var(--radius)` = 4px: "the scale
> used to step 8/6/4 px; the contrast pass collapses it so no component implies
> depth through corner size." The class names still differ in source but render
> identically — do not "fix" a `rounded-lg` to `rounded-sm` for visual reasons,
> and do not expect an 8px corner anywhere.
...
2. **Never `shrink-0` on a text element _without a width bound_.** `shrink-0`
   belongs on icons, badges, avatars, and fixed-width chrome. On text it is safe
   **only** when the element also carries a width bound (`w-20`, `max-w-*`) plus
   `truncate` — a bounded box cannot force the parent past the viewport, which is
   the only failure this rule exists to prevent. Unbounded `shrink-0` text is the
   defect. (`FirmLevelNetwork.tsx:1360` — `w-20 shrink-0 truncate` — is compliant.)
