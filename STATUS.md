...
**D71 partly closed — the first movement on `audit-actor` (G4) since WP 4.1.**

The ratchet listed sixteen `SECURITY DEFINER` writers as one debt. Reading each
signature splits them into three groups nothing had distinguished: **3** already
take `p_user_id` and never pass it on (one line each), **3** already attribute
through `assert_writer_may_act`, and **10** take no actor parameter at all — a
signature change plus every caller. §13 budgeted all sixteen as one-liners.

`apply_policy_bundle`, `assign_bom_line` and `assign_outbound_customer` are the
first group — `assign_material_supplier`'s shape three more times — and
`20260918000002` closes them with no signature change and no client change.

⚠️ **The bodies were copied mechanically, not retyped.** A `CREATE OR REPLACE`
that silently drops a line of a 105-line function is a regression no gate here
would catch: the artifact would record the new body and agree with itself. The
migration was generated by a script, and the diff against each original is
exactly the inserted block.

**`rehearsal/200` poisons the GUC with a stranger's id before every call**, so a
row naming the actor can only have come from the function. Its §4 is the half
that is easy to omit: `apply_policy_bundle`'s `p_user_id` is `DEFAULT NULL`, and
an unguarded `set_config` would blank an actor the caller's transaction had
already set — making the row LESS attributable than before the migration.

⚠️ **The remaining budget has been wrong twice.** D71 said seventeen live, D78
corrected it to six; measurement says **twelve of the thirteen were reachable**,
and nine of the ten left still are. That figure is no longer prose — the test
derives it and fails when §4 stops matching. The first draft of the counter
matched `rpc('<name>')` and reported the upload path's two item-master RPCs as
having no caller at all, because `useItemMasters.tsx` dispatches through
`sb.rpc(UPSERT_RPC[table], …)`.

⚠️ **A mutation survived, and for the third slice running it had disabled
nothing** — the script anchored on the first occurrence of `assign_bom_line`,
which is in the migration's own header comment. Re-anchored with an assert that
the text actually changed, it is caught. Seven mutations, all caught.

**`audit-actor` (G4) has NOT moved to met.** Ten writers still name no actor.
What moved: five functions attribute where three did, the debt is sized by a
count rather than a memory, and the ratchet now proves the one-line group is
empty — so the next reader cannot mistake ten signature changes for ten
one-liners.

**Verified:** red first · three rehearse modes green (19 assertion files) ·
`contract:check` ✓ · 409 tests ✓ (was 407) · `build` ✓ · eslint 336/116 and
`audit:ui` 8, unchanged.

**Needs a decision:** the ten remaining writers are ONE migration, not ten —
same change each, nine callers across four files. Ten separate slices would cost
ten migrations and ten rehearsals for one invariant. Also
`create_default_policy_defaults` is reachable from nothing at all and should not
be budgeted as work.
