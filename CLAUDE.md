...
| `input-hash` | I5 | Every derived row carries the input hash it came from | **not yet — and the OWNER in this row was wrong, which `20260917000002` found by finishing.** `20260917000002` completed the HASH: `graph_hash` covers every tier-2 value column, splits into `hash_inputs` / `hash_network`, and `graphHashCoverage.test.ts` fails when a column goes missing (D11, D67, D68 closed). But I5 is about the DERIVED ROW carrying it, and no tier-3 table has a `computed_from_hash` column — that is **WP 4.3**'s scope in §11, with `analysis_runs.input_hash` in **WP 4.2**. So the anchor a derived row would carry now exists and is complete; nothing carries it |
