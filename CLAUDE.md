...
| `result-binding` | I8 | Every result binds dataset + policy + scenario + engine version | **not yet** — WP 6.3. **Re-homed by `20260917000008`, whose package was never scoped to it**: §11 scopes that package to staleness and the Trust Report, and this row (plus ten table deferrals in `coverage.yaml`) pointed at it anyway — an owner nothing behind it had agreed to, which is exactly what R8 exists to catch. WP 6.3 ships the A5 Reproducibility Record, which IS this invariant stated as a deliverable |
...
of the same report). Migration first, then the probes, in separate pushes. **Measure every project, never just one** — the
