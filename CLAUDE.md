...
| `no-tier-skip` | I2 | No tier skipping — external data never lands below T1; pages never write T3 | **PARTIAL since WP 3.2.** The six CSV datasets the contract describes now land in tier 0 (`ingest_files`) and tier 1 (`ingest_staged_rows`) and reach tier 2 only through `ingest_apply_run`; `supabase/rehearsal/070` proves it against a real database and `ingestSpecParity.test.ts` fails if a `.split(',')` returns to the client. **Three groups still skip and each is named**: the item masters (D55, WP 3.3) and the node list and deep-tier network tables (D56, WP 4.2). No gate yet says a NEW dataset must land — the rule is enforced for the six by the absence of any other write path, not by a check |
