...
// Same vocabulary as Landing.tsx: mono kickers, #BF2330 accent, hairline grids,
// serif-italic tail on headings.
//
// ┌──────────────────────────────────────────────────────────────────────────┐
...
// Canonical micro-label style — mirrors Landing.tsx.
...
function FundingStrip() {
  ...
  return (
    <div className="relative border-t border-border bg-secondary/30">
      {/* hairline grid: the divider is the 1px gap, so it turns horizontal when the
          two columns stack instead of leaving a stray left border */}
      <div className="mx-auto grid max-w-6xl grid-cols-[repeat(auto-fit,minmax(330px,1fr))] gap-px bg-border">
        {/* Home — HWR / lab identity */}
        <div className="bg-background py-5 pl-6 pr-[30px]">
          <span className={KICKER}>Home</span>
          <img
            src="/funding/hwr-berlin.png"
            alt="HWR Berlin"
            className="mt-3.5 h-10 w-auto object-contain"
          />
        ...
        <div className="bg-background py-5 pl-[30px] pr-6">
              ...
              {PROGRAMMES.map((p, n) => (
                <img
                  ...
                  className={`absolute inset-0 h-full w-full object-contain object-left ${fade} ${
                    i === n ? 'translate-y-0 opacity-100' : 'translate-y-1.5 opacity-0'
                  }`}
                />
              ))}
            </div>
            ...
            {PROGRAMMES.map((p, n) => (
              ...
              >
                <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1.5">
                  <span className="text-[14.5px] leading-snug">{p.line}</span>
                  <span
                    className={`whitespace-nowrap rounded-sm border px-1.5 py-0.5 font-mono text-[9px] uppercase tracking-[0.18em] ${p.statusClass}`}
                  >
                    {p.status}
                  </span>
                </div>
                <span className="mt-1.5 block font-mono text-[11px] tracking-[0.05em] text-muted-foreground tabular-nums">
                  {p.detail}
                </span>
              </div>
            ))}
      ...
      <main className="flex-1">
        ...
        <section className="relative overflow-hidden border-b border-border">
          ...
          <div className="relative mx-auto max-w-6xl px-6 pt-14">
            <span className="inline-flex items-center whitespace-nowrap rounded-sm border border-border bg-card px-3 py-1.5">
              <span className="mr-2 h-1.5 w-1.5 rounded-full bg-[#BF2330]" />
              <span className={KICKER}>About us</span>
            </span>

            <div className="mt-7 flex flex-wrap items-end gap-x-[72px] gap-y-6 pb-14">
              <h1 className="flex-[1_1_520px] max-w-[680px] text-4xl font-semibold leading-[1.02] tracking-tight text-balance sm:text-5xl md:text-[62px]">
                Built inside a research lab,{' '}
                <span className="font-serif font-medium italic">shipped as a product.</span>
              </h1>
              <p className="flex-[1_1_320px] max-w-[440px] text-[16.5px] leading-relaxed text-muted-foreground text-pretty">
        ...
        <section id="people" className="border-b border-border">
          <div className="mx-auto max-w-6xl px-6 pb-14 pt-3.5">
            <div className="max-w-[600px]">
              <span className="inline-flex items-center gap-1.5 whitespace-nowrap font-mono text-[10px] uppercase tracking-[0.2em] text-[#BF2330]">
                <span className="h-1.5 w-1.5 rounded-full bg-[#BF2330]" />
                Key people
              </span>
              <h2 className="mt-3 text-[26px] font-semibold leading-tight tracking-tight sm:text-[32px]">
                The science and the build,{' '}
                <span className="font-serif font-medium italic">core team.</span>
              </h2>
            </div>

            <div className="mt-6 grid grid-cols-[repeat(auto-fit,minmax(370px,1fr))] items-stretch gap-px overflow-hidden rounded-md border border-border bg-border">
              {PEOPLE.map((p) => (
                <article
                  key={p.slug}
                  className="relative flex flex-col bg-card px-[34px] pb-7 pt-8"
                >
                  <span className="absolute right-[34px] top-8 font-mono text-[9px] tracking-[0.16em] text-muted-foreground/60">
                    {p.num}
                  </span>

                  {/* overflow-hidden contains the float; the bio clears it */}
                  <div className="overflow-hidden">
                    <div className="float-left mr-[18px] mb-2.5 mt-0.5 h-[116px] w-[92px] overflow-hidden rounded-sm bg-secondary">
                      <img
                        src={p.photo}
                        alt={p.name}
                        loading="lazy"
                    ...
                    <span className="block whitespace-nowrap font-mono text-[10px] uppercase tracking-[0.2em] text-[#BF2330]">
                      {p.role}
                    </span>
                    <h3 className="mt-3 text-[22px] font-semibold leading-[1.22] tracking-tight text-balance">
                      {p.name}
                    </h3>
                    <p className="mt-2 text-sm leading-snug text-foreground/80 text-pretty">
                      {p.title}
                    </p>
                  ...
                  <div className="mt-6 flex flex-wrap gap-[7px]">
                    {p.tags.map((t) => (
                      <span
                        key={t}
                        className="whitespace-nowrap rounded-sm border border-border px-2.5 py-1 text-xs text-foreground/70"
                      >
                        {t}
                      </span>
                    ))}
