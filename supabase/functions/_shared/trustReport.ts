 ...
 *
 * ── AND WP 6.3 MOVED IT HERE, WHICH IS WHAT "WITHOUT A SECOND IMPLEMENTATION"
 *    ACTUALLY COSTS ──────────────────────────────────────────────────────────
 *
 * The module was authored under `src/lib/trust/`, where only the browser could
 * reach it. A3 renders through `report-render`, a Deno edge function, so the
 * report has to be computable WHERE THE DATA IS — otherwise the PDF is built from
 * numbers the browser asserted, and T4 ("reproducible or not published") is a
 * claim about a client. Rather than write a server copy, the module moved to
 * `_shared/`, which both sides already import (`grading.ts`,
 * `ingestSpec.generated.ts`), and `src/lib/trust/trustReport.ts` is now a
 * re-export so every existing consumer is untouched.
 *
 * The type import moved with it: the shapes now come from `grading.ts` itself
 * rather than from `src/lib/trust/gradingTypes.ts`, which existed to mirror them
 * across a boundary this module no longer sits on.
 */
