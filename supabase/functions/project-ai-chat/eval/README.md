...
| `closed_loop_test.ts` + `fixtures/closed-loop/cl-01…cl-08` | §20.6 closed-loop suite (H2, `CLOSED_LOOP_ENABLED`; cl-04/09/10 land with H3): the §20.2 cache-first read (hit table + 12-hex triple note + derived badge, `cache_miss` vs `cache_stale` naming the drifted hash, disambiguation, reps-insufficient miss), the deterministic `cache_hit` draft guard (cl-07 — the guard, not the prompt, is the gate), the §20.3 single-turn branches on the real B4 turn, read-hit ≡ apply-hit through the ONE extracted G17 predicate (`findReuseCandidates`), and the §13.6 stub-DB laws (zero run rows without approval; zero proposals on hits); flag off ⇒ the §5.4 v1 surface/prompt/behavior byte-identically. |
