...
| `disruption_sentinel_test.ts` + `fixtures/disruption-sentinel/ra-01…ra-08` | §18.3 golden task suite (Phase 4d): the sensing-registry guard + the Q28 corroboration mechanics (authoritative = sensing ∩ grade A, enumerated; verified ⇔ ≥1 authoritative OR ≥3 independent sources), the deterministic event typer, evidence persistence on the shared B8 substrate, deep-tier exposure matching with citable map evidence, the LINKED B4 spec through the registered `draft_experiment_spec` handler, injection-as-data on the sensing path (ra-06), the §4.4 apply through `applyExperimentSpec` → `dispatchExperimentRun` with full provenance stamps + `stale_values` re-verification, and the impact law (ranges byte-equal to `run_replications` recomputation, run citation, filled exactly once — never model-supplied). |
| `demo_disruption_sentinel.ts` | Runnable Phase 4d acceptance transcript: a pasted plant-fire article naming a MAPPED tier-2 supplier → corroboration gathered across three independent sensing sources → alert card with matched entities (via the B8 map, citable) + linked experiment spec + PENDING impact → Approve → run dispatched via the B4 path (full stamps, worker envelope) → after completion the alert shows the simulated impact range with its run citation. |
