...
-- holds **11 of the 16** — five exist in production and in no migration (§4 D158), so a
...
BEGIN
  ...
  IF array_length(v_narrow, 1) IS NOT NULL THEN
    RAISE EXCEPTION
      'WP 7.1/300 §1: % policy/policies still do not name both roles: % — stage 1 would refuse these reads to whoever logged in (D155)',
  ...
  IF v_rows <> 0 THEN
    RAISE EXCEPTION
      'WP 7.1/300 §2: authenticated read % row(s) through a policy granted TO anon ALONE — the premise of this whole stage is wrong and D155 must be re-measured',
      v_rows;
  END IF;
