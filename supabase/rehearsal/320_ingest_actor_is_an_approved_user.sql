...
-- whole point of D131. Every other file that exercises the landing INSERTs its actor into
...
BEGIN
  ...
  INSERT INTO public.approved_users (id, name, email, password_hash, role) VALUES
    ...
    -- there): the uploader from §1 cannot be deleted at all, which is D136.
  ...
  IF v_rows <> 0 THEN
    RAISE EXCEPTION
      'WP 6.5a/320 §2: ingest_runs still carries % foreign key(s) to auth.users — a real uploader cannot be recorded (D131)',
      v_rows;
  END IF;
  ...
  --
  -- (2) The run used here is one with NO FILE, and that is not tidiness — it is §4 D136.
  ...
  -- ══ §4 · D136, PINNED AS THE COLLISION IT IS ══
  --
  ...
  BEGIN
    ...
    RAISE EXCEPTION
      'WP 6.5a/320 §4: a user with an ingest_files row WAS deleted. D136 says the tier-0 write-once trigger refuses the SET NULL that deletion requires — if that changed, the tier-0 immutability rule or the key changed with it, and this file needs rewriting rather than passing.';
  ...
  RAISE NOTICE 'WP 6.5a/320 §4: a user who has uploaded a file cannot be deleted — D136, pinned not fixed';

  RAISE NOTICE 'WP 6.5a · 320: a real uploader can land, both keys point at approved_users, and the record outlives the person; D136 pinned — 4 section(s)';
END $wp65pre$;
