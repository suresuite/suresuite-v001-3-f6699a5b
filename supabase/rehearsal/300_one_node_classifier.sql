...
--       have (§4 D127). `ProductLevelNetwork`'s answer for such a node depends on
...
--   §3  the deep-tier lane is in the projection (§4 D132): a node that exists ONLY
...
--       §4 D143, a trigger that read NEW in a statement-level context and therefore
...
--     which is §4 D134's substitution caught in a second column.
...
BEGIN
  ...
  -- node §4 D127 says four classifiers answer four ways.
  ...
  -- ── 3 · the deep-tier lane is inside the projection (D132) ──────────────
  --
  ...
  IF v_n <> 1 THEN
    RAISE EXCEPTION
      ...
      'lane trigger did not fire (D132 / D143).', v_n;
  END IF;
  SELECT echelon INTO v_e FROM public.node_list
   WHERE project_id = v_project AND node_id = 'DEEPONLY';
  ...
  -- to 0 rather than staying NULL, §4 D134 has been rebuilt in a new column: an
  ...
  IF v_tier IS NOT NULL THEN
    RAISE EXCEPTION
      ...
      'supply_tier % instead of NULL. An unknown answered with a number is D134.', v_tier;
  END IF;
  ...
  -- (`network_nodes.uid` against material ids is §4 D137) and is not this package's.
  ...
  -- ── 5 · idempotent, and the trigger that never fired now does (D143) ────
  --
  ...
  IF v_n <> 1 THEN
    RAISE EXCEPTION
      ...
      'out of the typed projection. The lane trigger is dead again — D143.';
  END IF;
  ...
  RAISE NOTICE
    ...
    'trigger fires for the first time since 2025-08-29 (D143).';
END
$wp81$;
...
BEGIN
  ...
  -- flattened `level` cannot provide (§4 D140).
