-- Remove the problematic trigger and function causing the issue
DROP TRIGGER IF EXISTS set_simulation_results_defaults ON public.simulation_results;

-- Create a proper default function for simulation_results if needed
CREATE OR REPLACE FUNCTION public.set_simulation_results_defaults()
RETURNS TRIGGER AS $$
DECLARE
  current_org text;
BEGIN
...
-- Create the proper trigger for simulation_results
CREATE TRIGGER set_simulation_results_defaults
  BEFORE INSERT ON public.simulation_results
  FOR EACH ROW
  EXECUTE FUNCTION public.set_simulation_results_defaults();
