...
--
-- §4 D127: a node's TYPE is authored eight times — once in SQL, four times across
...
-- `stale`, and a depth nobody knows is not a depth of 0 (§4 D134 is exactly what
...
--     disagree about what that column means (§4 D140) and choosing between them is
...
COMMENT ON COLUMN public.node_list.echelon IS
  'WP 8.1 · §4 D127. The node''s ROLE in the supply chain, derived once by '
...
COMMENT ON COLUMN public.node_list.bom_depth IS
  'WP 8.1 · §4 D127. Depth in the BOM tree, from `bom_multi_level.level` — the '
...
COMMENT ON COLUMN public.node_list.supply_tier IS
  'WP 8.1 · §4 D127. Tiers upstream of the focal plant: 0 the plant, 1 a direct '
  ...
  'suppliers, and reporting 0 for them would be the D134 substitution again. This '
...
-- projection in the repository. §15 measured 104 such nodes (§4 D132).
--
...
COMMENT ON FUNCTION public.classify_node_echelon(uuid, text) IS
  'WP 8.1 · §4 D127. THE node classifier. Every other one in this repository is '
  ...
  '104 deep-tier nodes untyped (§4 D132).';
...
COMMENT ON FUNCTION public.classify_node_type(uuid, text) IS
  'WP 8.1 · §4 D127. The LEGACY four-value vocabulary, now DERIVED from '
...
AS $function$
  ...
  -- `supply_chain_data_multi_tier.level`, which two writers disagree about (D140)
...
AS $function$
  ...
  -- would be D134's substitution wearing a different column name.
  --
...
-- repository has (§4 D132, 104 nodes). Widening the CTE is the easy half.
--
...
-- from explicit client calls. The moment D143's fix makes the LANE trigger fire
...
COMMENT ON FUNCTION public.node_list_discover(uuid, text, uuid) IS
  'WP 8.1 · §4 D132, D66. The node-set discovery, reading BOTH edge tables and '
...
COMMENT ON FUNCTION public.refresh_node_list_for_project(uuid, uuid) IS
  'WP 8.1 · §4 D127, D132. Rebuilds the typed node projection from BOTH edge '
...
-- ── 7 · BOTH lane triggers, AND D143: the one that exists has never fired ──
--
...
--
-- That is **§4 D143**, and it is D80's shape exactly: a whole mechanism whose
-- failure has no symptom, because a refresh that does not happen looks like a
-- graph that has not changed. It is also the other half of the answer to D132 —
...
COMMENT ON FUNCTION public.auto_refresh_node_list_on_lane_change() IS
  'WP 8.1 · §4 D143. Replaces `auto_refresh_node_list_on_scd_change`, whose body '
...
-- `supply_chain_data_multi_tier` — the lane nothing has ever watched (D132).
...
COMMENT ON FUNCTION public.auto_refresh_node_list_on_scd_change() IS
  'DEAD as of WP 8.1 — §4 D143. Read NEW/OLD in a STATEMENT-level trigger, so the '
...
-- changes, and §4 D127's row says why.
UPDATE public.node_list nl
SET echelon     = e.echelon,
    node_type   = CASE e.echelon WHEN 'subassembly' THEN 'material' ELSE e.echelon END,
    node_group  = initcap(CASE e.echelon WHEN 'subassembly' THEN 'material' ELSE e.echelon END),
