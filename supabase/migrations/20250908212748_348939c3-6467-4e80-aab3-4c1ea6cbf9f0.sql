-- Create or replace the function to combine datasets and also build multi-tier path data
...
BEGIN
  ...
  -- Helper: inbound shares per plant/material
  WITH shares AS (
    SELECT 
      i.project_id,
      i.plant_name,
  ...
  -- 8) Multi-level BOM per product-customer pair (levels >= 1)
  ...
  -- Insert multi-level BOM edges (child -> parent) per path
  INSERT INTO public.supply_chain_data_multi_tier (
    project_id, plant_name, data_source,
    from_location, to_location, weighted, level, path_root, organization, uploaded_by
  )
  ...
  -- Insert Supplier -> Material edges per path using inbound shares
  INSERT INTO public.supply_chain_data_multi_tier (
    project_id, plant_name, data_source,
    from_location, to_location, sourcing_ratio, weighted, level, path_root, organization, uploaded_by
  )
