-- Fix the trigger that's incorrectly attached to simulation_results table
-- The set_supply_chain_data_defaults() function should only be on supply_chain_data table

-- Remove any incorrect triggers on simulation_results
DROP TRIGGER IF EXISTS trigger_set_supply_chain_data_defaults ON public.simulation_results;

-- Ensure the trigger is properly attached only to supply_chain_data table
DROP TRIGGER IF EXISTS trigger_set_supply_chain_data_defaults ON public.supply_chain_data;
CREATE TRIGGER trigger_set_supply_chain_data_defaults
  BEFORE INSERT ON public.supply_chain_data
  FOR EACH ROW
  EXECUTE FUNCTION public.set_supply_chain_data_defaults();

-- Now insert the simulation placeholder data
