-- Fix CTE scoping in combine_project_into_supply_chain for section 8 (duplicate CTEs per INSERT)
...
BEGIN
  ...
  -- 8a) Multi-level BOM per product-customer pair (levels >= 1) - BOM edges per path
  ...
  -- 8b) Supplier -> Material edges per path using inbound shares (duplicate CTE scope)
  WITH RECURSIVE op AS (
    SELECT 
      o.project_id,
      o.plant_name,
