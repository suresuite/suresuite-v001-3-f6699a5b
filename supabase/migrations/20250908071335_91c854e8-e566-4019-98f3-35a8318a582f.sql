-- Remove network_summary from dataset status and completion RPCs
...
-- Align RPC update_project_completion_status to same logic (no network_summary)
...
BEGIN
  ...
  IF deep_tier_enabled THEN
    SELECT EXISTS(SELECT 1 FROM public.network_nodes WHERE project_id = p_project_id) INTO has_deep_tier_nodes;
    SELECT EXISTS(SELECT 1 FROM public.network_edges WHERE project_id = p_project_id) INTO has_deep_tier_edges;
    final_completed := (has_bom AND has_inbound AND has_outbound AND has_deep_tier_nodes AND has_deep_tier_edges);
  ELSE
