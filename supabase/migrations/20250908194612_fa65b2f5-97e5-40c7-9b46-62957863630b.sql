-- Harden combine_project_into_supply_chain to prevent NULL to_location insertions
...
BEGIN
  ...
  -- Insert inbound logistics: supplier -> material (defensive NULL checks)
  ...
  -- Insert outbound logistics: product -> customer (defensive NULL checks)
  INSERT INTO public.supply_chain_data (
    project_id, plant_name, data_source, data_source_group,
    from_location, to_location, material_consumption_rate, sourcing_ratio, weighted
  )
  ...
  IF v_bom_level = 'single' THEN
    -- Single-level: material -> product (prevent NULL product_id)
    INSERT INTO public.supply_chain_data (
      project_id, plant_name, data_source, data_source_group,
      from_location, to_location, material_consumption_rate, sourcing_ratio, weighted
    )
  ...
  ELSE
    -- Multi-level: material -> higher_level_component (prevent NULL higher_level_component_id)
    INSERT INTO public.supply_chain_data (
      project_id, plant_name, data_source, data_source_group,
      from_location, to_location, material_consumption_rate, sourcing_ratio, weighted
    )
