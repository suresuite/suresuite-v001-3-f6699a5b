...
-- Target project fetched earlier
-- If you rerun, this uses idempotent guard to avoid duplicates by scenario name markers in metrics
WITH proj AS (
  SELECT 
    '48e9f6e0-2fee-4318-be0b-33b8ea353643'::uuid AS project_id,
    'Plan 3'::text AS plant_name,
    'Company1'::text AS organization,
    '6fb76f62-876b-4616-b926-006691742c49'::uuid AS modeler_id
)
...
SELECT 
  ...
  $$
  {
    ...
    "available_kpis": [
      "fill_rate", "revenue", "profit", "delivery_on_time", "backlog", "resilience_cost"
    ],
    "baseline": {
      "fill_rate": [
...
FROM proj p
WHERE NOT EXISTS (
  SELECT 1 FROM public.simulation_results sr 
  WHERE sr.project_id = p.project_id AND sr.plant_name = p.plant_name AND sr.metrics->>'label' = 'Supplier Factory Fire'
);
...
WHERE NOT EXISTS (
  SELECT 1 FROM public.simulation_results sr 
  WHERE sr.project_id = p.project_id AND sr.plant_name = p.plant_name AND sr.metrics->>'label' = 'Port Strike Disruption'
);
...
WHERE NOT EXISTS (
  SELECT 1 FROM public.simulation_results sr 
  WHERE sr.project_id = p.project_id AND sr.plant_name = p.plant_name AND sr.metrics->>'label' = 'Cyber Attack on Systems'
);
