-- WP 6.5a precondition / §4 D131 — the ingestion actor points at the table it comes from
--
...
-- ── TWO NAMES FOR ONE KEY, WHICH IS §4 D135 ────────────────────────────────
--
...
COMMENT ON COLUMN public.ingest_runs.triggered_by_user_id IS
  ...
  'precondition (§4 D131) — it pointed at auth.users, where no user of this application '
...
COMMENT ON COLUMN public.ingest_runs.applied_by_user_id IS
  ...
  'correction as triggered_by_user_id (§4 D131).';
