-- Fix ambiguous function resolution when seeding synthetic data
-- 1) Drop the older, duplicate create_disruption_scenario_v2 that lacks disruption dates
DROP FUNCTION IF EXISTS public.create_disruption_scenario_v2(
  uuid,
  text,
  text,
  uuid,
...
-- 2) Update seed_synthetic_simulation_data to cast status and explicitly pass disruption dates
